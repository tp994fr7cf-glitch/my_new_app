...
■ 【重要・最新】PR #61 マージ後もユーザーが「何も変化していません」と報告（2026-07-08）
- git pull・flutter clean・flutter pub get・flutter run をすべて実施済みで、Android
  エミュレータでも本番 Web（デプロイ成功済み・キャッシュ自動クリア済み）でも症状(A)が同じ
  ように再発するとのこと
- コードレビュー・自動テスト（症状(A)の手順そのままを再現するテストを含む）はすべて
  「修正は正しい」という結果で、静的調査では原因を特定できなかった
- そのため PR #62 で、動作を変えない一時的な調査用ログ（[LessonMediaSwitchDebug] プレフィックス）
  を lesson_media_playlist_playback.dart / lesson_media_playback.dart に追加した
  （seekGlobal、_seekGlobalImmediate、_activateSegment、_prepareSegmentInSlot、
  _preloadNextSegment、play/pause、実プレイヤーの open/seek/play/pause など）
- 次にやるべきこと: ユーザーに症状(A)の再現操作をしてもらい、その間の flutter run の
  ターミナル出力（[LessonMediaSwitchDebug] の行）を貼ってもらって、実際に何が起きているかを
  ログから特定する。静的解析でロジックが正しく見えても実機だけで起きる何かがある可能性が高い
- 合わせて疑うべき点: lesson.mediaSegments の実データが想定（音声0-90秒・動画90-180秒）と
  本当に一致しているか。video_lesson_page.dart で _playlistPlayback が想定通り1つだけ
  生成されているか
