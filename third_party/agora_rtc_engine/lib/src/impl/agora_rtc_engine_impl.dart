...
extension RtcEngineExt on RtcEngine {
  ...
  Future<void> _setupRemoteVideoExCompat(
    ...
    return (this as RtcEngineImpl).setupRemoteVideoEx(
      canvas: canvas,
  ...
  Map<String, dynamic> _createParamsWeb(
    Object viewHandle,
    VideoCanvas canvas, {
    RtcConnection? connection,
  }) {
    ...
    switch (eventName) {
      ...
      case 'onLeaveChannel_c8e730d':
        ...
        if (connection?.channelId != null) {
          ...
          ChannelConnectionManager.instance.removeConnection(
            connection.channelId!,
          );
        }
        break;
...
class RtcEngineImpl extends rtc_engine_ex_binding.RtcEngineExImpl
  ...
  GlobalVideoViewControllerPlatfrom get globalVideoViewController {
    _globalVideoViewController ??= createGlobalVideoViewController(
      irisMethodChannel,
      this,
    );
  ...
  Future<void> _initializeInternal(RtcEngineContext context) async {
    // This app uses autoRegisterAgoraExtensions=false for its audio-only RTC
    // path. In that mode there will never be an AgoraVideoView, so creating the
    // Iris video frame buffer manager is unnecessary. On Android 6.6.3 this
    // native call can remain pending on the first process launch and prevent
    // the audio engine itself from initializing.
    if (context.autoRegisterAgoraExtensions == false) {
      return;
    }
    await globalVideoViewController.attachVideoFrameBufferManager(
  ...
  Future<void> initialize(RtcEngineContext context) async {
    ...
    await irisMethodChannel.invokeMethod(
      IrisMethodCall('RtcEngine_setAppType', jsonEncode({'appType': 4})),
    );
  ...
  void registerEventHandler(
    covariant RtcEngineEventHandler eventHandler,
  ) async {
  ...
  Future<MediaPlayer?> createMediaPlayer() async {
    ...
    final param = createParams({});
    final callApiResult = await irisMethodChannel.invokeMethod(
      IrisMethodCall(apiType, jsonEncode(param)),
    );
    if (callApiResult.irisReturnCode < 0) {
      return null;
    }
  ...
  Future<void> destroyVideoEffectObject(
    covariant VideoEffectObject videoEffectObject,
  ) async {
  ...
  void registerMediaMetadataObserver({
    required MetadataObserver observer,
  ...
  Future<int> getNativeHandle() async {
    ...
    final nativeHandleIntPtr = resultStr.substring(
      resultStr.indexOf(':') + 1,
      resultStr.length - 1,
    );
...
class VideoDeviceManagerImpl extends rtc_engine_binding.VideoDeviceManagerImpl
  ...
  factory VideoDeviceManagerImpl.create(RtcEngine rtcEngine) {
    return rtcEngine.objectPool.putIfAbsent(
      _videoDeviceManagerKey,
      () => VideoDeviceManagerImpl._(rtcEngine),
    );
  }
